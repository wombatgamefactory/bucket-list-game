// Bucket List: Expert AI Bot
// Strategy: extend sequences optimally; intelligently minimize damage; preserve flexibility
...
function countFutureFlexibility(grid, cellIndex) {
  const row = Math.floor(cellIndex / 6);
  const col = cellIndex % 6;
...
export function expertDraft(gameState) {
...
export function expertPlace(gameState, validCells) {
  const player = gameState.players[gameState.currentPlayerIndex];
  const grid = player.grid;
  ...
  if (productiveCells.length > 0) {
    ...
    for (const cellIndex of productiveCells) {
      ...
      const flexibility = countFutureFlexibility(grid, cellIndex);

      if (scoreDelta > bestScoreDelta) {
        bestScoreDelta = scoreDelta;
        bestFlexibility = flexibility;
        bestCell = cellIndex;
      } else if (scoreDelta === bestScoreDelta && flexibility > bestFlexibility) {
        bestFlexibility = flexibility;
        bestCell = cellIndex;
      } else if (scoreDelta === bestScoreDelta && flexibility === bestFlexibility && cellIndex < bestCell) {
        bestCell = cellIndex;
      }
    }
  ...
  // No productive placements: minimize damage AND preserve flexibility
  ...
  for (const cellIndex of validCells) {
    ...
    // Prioritize: least damage > most flexibility > position
    if (damage < leastDamage) {
      leastDamage = damage;
      bestFlexibility = flexibility;
      bestCell = cellIndex;
    } else if (damage === leastDamage && flexibility > bestFlexibility) {
      bestFlexibility = flexibility;
      bestCell = cellIndex;
    } else if (damage === leastDamage && flexibility === bestFlexibility && cellIndex < bestCell) {
      bestCell = cellIndex;
    }
  }
